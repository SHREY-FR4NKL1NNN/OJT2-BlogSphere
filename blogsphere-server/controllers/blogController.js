...
exports.updateComment = async (req, res) => {
  try {
    ...
    res.status(500).json({ msg: "Failed to update comment", error: err.message })
  }
}
...
exports.getBlogLikes = async (req, res) => {
  try {
    const blog = await Blog.findById(req.params.id)
    if (!blog) return res.status(404).json({ msg: "Blog not found" })
    const liked = req.user ? blog.likes.some(uid => uid.toString() === req.user.id) : false
...
exports.getComments = async (req, res) => {
  try {
    ...
    res.status(500).json({ msg: "Failed to fetch comments", error: err.message })
  }
}

exports.searchBlogs = async (req, res) => {
  try {
    const { q, following } = req.query
    const query = {}
...
exports.personalizedFeed = async (req, res) => {
  try {
    ...
    if (blogs.length === 0) {
      blogs = await Blog.find().populate("author", "username").sort({ createdAt: -1 })
    }
    res.json(blogs)
  } catch (err) {
    res.status(500).json({ msg: "Personalized feed failed", error: err.message })
  }
}
